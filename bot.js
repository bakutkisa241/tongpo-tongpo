...
bot.on("ready", async () => {
    ...
    function randomStatus() {
        let status = [`${bot.guilds.size} guilds.`, `${bot.guilds.reduce((a, b) => a + b.memberCount, 0).toLocaleString()} users.`, "b!help | Bolt", "bot.discord.io/bolt", "24/7 | Vote"]
          let rstatus = Math.floor(Math.random() * status.length);
        bot.user.setActivity(status[rstatus], {type: 'STREAMING', url: "https://www.twitch.tv/brickmaster__"});
...
bot.on("guildMemberRemove", async (member, client, message, args, level) => {  
 ...
 let welcomesetting = JSON.parse(fs.readFileSync("./welcomeonoff.json", "utf8"));
    ...
    if (!channel) return;
            ...
            channel.send(`**GoodBye** ${member} **See You... **`)+channel.send(new Discord.Attachment(await createCanvas()));
    }
});
